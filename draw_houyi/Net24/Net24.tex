...
\pic[shift={(0,10,0)}] at (node43-north) 
    {Ball={
        name=sum,
        fill=\SumColor,
        opacity=0.6,
...
\pic[shift={ (0,8,0) }] at (cascade-north) 
    {RightBandedBox={
        name=fc1,
        caption=fc1,
        xlabel=1,
...
\draw [connection]  (fc1-north)    -- node {\midarrow} (fc2-south);

\pic[shift={(0,5,0)}] at (fc2-north) 
    {Box={
        name=softMax,
        caption=SoftMax,
        xlabel=1,
...
\draw [connection]  (fc2-north)    -- node {\midarrow} (softMax-south);
